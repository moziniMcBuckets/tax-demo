...
def test_create_event(
    ...
    try:
        ...
        payload = [
            ...
            {
                'conversational': {
                    'content': {'text': "I don't have access to real-time weather data, but I can help you find weather information."},
                    'role': 'ASSISTANT'
                }
            }
        ]
...
def test_get_event(
    ...
    try:
        ...
        if not events:
            print(f"{Fore.YELLOW}✓ Test 3 skipped (no events available){Style.RESET_ALL}")
            return True
        ...
        print_msg(f"Test 3 failed - {e.response.get('Error', {}).get('Code', 'Unknown')}", "error")
        return False
    except Exception as e:
...
def test_pagination(
    ...
    try:
        ...
        print_msg(f"Test 4 failed - {e.response.get('Error', {}).get('Code', 'Unknown')}", "error")
        return False
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if "ResourceNotFoundException" in error_code or "NotFound" in error_code:
                print_msg("Test 6 passed - Invalid memory ID correctly rejected", "success")
...
Examples:
  ...
  uv run scripts/test-memory.py --memory-arn arn:aws:bedrock-agentcore:us-east-1:123456789:memory/abc123
    ...
    parser.add_argument(
        "--memory-arn",
        type=str,
        help="Memory ARN to use for testing"
    )
