...
"""
Escalation Manager Lambda - Mark clients for accountant intervention.
"""
...
def get_accountant_info(accountant_id: str) -> Dict[str, Any]:
    ...
    try:
        response = table.get_item(
            Key={'accountant_id': accountant_id, 'settings_type': 'contact_info'}
        )
        if 'Item' in response:
            return response['Item']
        return {'accountant_id': accountant_id, 'email': None, 'phone': None, 'name': 'Accountant'}
    except ClientError as e:
...
def log_escalation(client_id: str, reason: str, accountant_id: str, notification_sent: bool) -> str:
...
def send_email_notification(accountant_email: str, client_info: Dict[str, Any], reason: str) -> Optional[str]:
...
Action Required: Please contact this client directly.
...
def send_sns_notification(client_info: Dict[str, Any], reason: str) -> Optional[str]:
    ...
    try:
        response = sns.publish(
            ...
            Subject=f"Client Escalation: {client_info.get('client_name', 'Unknown')}",
            MessageAttributes={'event_type': {'DataType': 'String', 'StringValue': 'client_escalation'}}
        )
...
def get_escalation_reason_details(client_id: str, custom_reason: Optional[str] = None) -> str:
    ...
    try:
        ...
        last_date = followups[0].get('sent_date', 'Unknown')
        
        if last_date != 'Unknown':
            days_since = (datetime.utcnow() - datetime.fromisoformat(last_date)).days
        else:
            days_since = 0
        
        return f"No response after {followup_count} reminders over {days_since} days. Last contact: {last_date[:10]}. Client requires direct phone call."
    except ClientError as e:
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if notify_accountant:
            ...
            sns_message_id = send_sns_notification(client_info=client_info, reason=escalation_reason)
        ...
        response = {
            ...
            'next_action': 'Accountant should contact client directly'
        }
